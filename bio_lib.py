...
def save_uploaded_file(form):
    ...
    file_val = open(file_name, 'w')
    file_val.write(file_dat.read()) 
    file_val.close() 
    # Calculation setup.
...
def write_pqr(target, pqr):
    pqr_file = open(target + '-reo.pqr', 'w')
    pqr_file.write(pqr)
    pqr_file.close() 
...
def fix_pdb(target):
    ...
    # <<PATH>>
    if not os.path.exists(target + '-fix.pdb'):
        awk_cmd = ['awk', '/(ATOM|^TER)/,//']
        #awkp = Popen(awk_cmd, stdin=open('./' + target + '.pdb', 'r'),
        awkp = Popen(awk_cmd, stdin=open('./' + target + '-rec.pdb', 'r'),
                     stdout=PIPE, stderr=PIPE, shell=False)
        nat = open(target + '-nat.pdb', 'w')
...
def rechain(target):
    ...
    for rechaining."""
    if not os.path.exists(target + '-rec.pdb'):
        awk_cmd = ['awk', '/(ATOM|^TER|ENDMDL|MODEL)/,//']
        awkp = Popen(awk_cmd, stdin=open('./' + target + '.pdb', 'r'),
                     stdout=PIPE, stderr=PIPE, shell=False)
        val = awkp.stdout.readlines()
        rec = open('./' + target + '-rec.pdb', 'w')
...
def prepare_html(target, pqr, pH, Q_tot): #, comment): #, mode):
...
def prepare_Jmol(target):
    ...
    jmol_instr_dat = open('mod_template.spt', 'r')
